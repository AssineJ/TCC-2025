...
def verificar_noticia():
    ...
    try:
        ...
        if tipo == "url":
            ...
            log_info("Validando qualidade do texto...")
            validacao_texto = validar_qualidade_texto(conteudo)

            log_info(
                f"   Score qualidade: {validacao_texto['score_qualidade']}"
            )
                    ...
                    jsonify(
                        {
                            ...
                            "score_qualidade": validacao_texto[
                                "score_qualidade"
                            ],
            ...
            log_info(
                f"Texto validado com sucesso "
                f"(qualidade: {validacao_texto['score_qualidade']})"
            )
        ...
        if tipo == "url":
            ...
            log_info(
                f"Conteúdo extraído: {len(texto_para_analise)} caracteres"
            )
        ...
        log_info("Processando texto com IA...")
        resultado_nlp = processar_texto(texto_para_analise)

        log_info("NLP concluído:")
        log_info(f"   - {len(resultado_nlp['entidades'])} entidades encontradas")
        log_info(
            f"   - {len(resultado_nlp['palavras_chave'])} palavras-chave extraídas"
        )
        log_info(f"   - Query de busca: {resultado_nlp['query_busca']}")

        log_info("Buscando nas fontes confiáveis...")
        resultado_busca = buscar_noticias(resultado_nlp["query_busca"])

        log_info("Busca concluída:")
        log_info(
            f"   - Total de resultados: "
            f"{resultado_busca['metadata']['total_resultados']}"
        )
        log_info(
            f"   - Fontes com sucesso: "
            f"{resultado_busca['metadata']['fontes_com_sucesso']}/"
            f"{resultado_busca['metadata']['total_fontes']}"
        )

        resultado_busca_filtrado = filtrar_busca(resultado_busca)

        log_info("Filtros aplicados (busca):")
        log_info(
            f"   - Mantidos: "
            f"{resultado_busca_filtrado['metadata']['total_resultados']}"
        )
        log_info(
            f"   - Filtrados: "
            f"{resultado_busca_filtrado['metadata'].get('total_filtrados', 0)}"
        )

        log_info("Extraindo conteúdo das notícias encontradas...")
        resultado_scraping = scrape_noticias(resultado_busca_filtrado)

        log_info("Scraping concluído:")
        log_info(
            f"   - Total processado: "
            f"{resultado_scraping['metadata']['total_scraped']}"
        )
        log_info(
            f"   - Sucessos: "
            f"{resultado_scraping['metadata']['total_sucesso']}"
        )
        log_info(
            f"   - Taxa: "
            f"{resultado_scraping['metadata']['taxa_sucesso']:.1f}%"
        )

        resultado_scraping_filtrado = filtrar_scraping(
            resultado_scraping, texto_para_analise
        )

        log_info("Filtros aplicados (scraping):")
        log_info(
            f"   - Mantidos: "
            f"{resultado_scraping_filtrado['metadata']['total_sucesso']}"
        )
        log_info(
            f"   - Filtrados: "
            f"{resultado_scraping_filtrado['metadata'].get('total_filtrados', 0)}"
        )

        log_info("Analisando similaridade semântica com IA...")
        resultado_analise = analisar_semantica(
            texto_para_analise, resultado_scraping_filtrado
        )

        log_info("Análise semântica concluída:")
        ...
        log_info(
            f"   - Confirmam forte: "
            f"{resultado_analise['metadata']['confirmam_forte']}"
        )
        log_info(
            f"   - Confirmam parcial: "
            f"{resultado_analise['metadata']['confirmam_parcial']}"
        )
        log_info(
            f"   - Apenas mencionam: "
            f"{resultado_analise['metadata']['apenas_mencionam']}"
        )

        log_info("Calculando veracidade final...")
        resultado_score = calcular_veracidade(
            resultado_analise,
            {
                "tipo_entrada": tipo,
                "tamanho_conteudo": len(texto_para_analise),
                "total_fontes_buscadas": resultado_busca["metadata"][
                    "total_resultados"
                ],
            },
        )
        ...
        log_info(
            f"   Nível de confiança: {resultado_score['nivel_confianca']}"
        )

        if contradizem > 0:
            log_info(
                f"     ALERTA: {contradizem} fonte(s) contradizem a informação!"
            )
        ...
        for fonte_nome, fonte_analises in resultado_analise.items():
            ...
            for analise in fonte_analises:
                if analise.get("sucesso"):
                    ...
                    if (
                        len(url_analise) < 80
                        and titulo_chave in urls_originais_busca
                    ):
                        url_final = urls_originais_busca[titulo_chave]
                        log_info(
                            f"[RECUPERADO] URL completa de {fonte_nome}: "
                            f"{len(url_final)} chars"
                        )
                    else:
                    ...
                    fontes_consultadas.append(
                        {
                            ...
                            "confianca_contradicao": analise.get(
                                "confianca_contradicao", 0.0
                            ),
                        }
                    )

        fontes_consultadas.sort(
        ...
        resposta = {
            ...
            "metadata": {
                ...
                "total_resultados_busca": resultado_busca["metadata"][
                    "total_resultados"
                ],
                "total_scraped": resultado_scraping_filtrado["metadata"][
                    "total_scraped"
                ],
                "total_analisados": meta_analise["total_analisados"],
        ...
        log_info(
            ...
            f"veracidade={resposta['veracidade']}% | "
            f"fontes={len(fontes_consultadas)}"
        )

        if contradizem > 0:
            log_info("ALERTA FINAL: Detectada provável FAKE NEWS!")
        ...
        jsonify(
            {
                ...
                "descricao": (
                    "Sistema de Verificação de Veracidade com IA e "
                    "Validação de Texto"
                ),
                "endpoints": {
                ...
                "fontes_confiaveis": [
                    fonte["nome"] for fonte in Config.TRUSTED_SOURCES
                ],
                "tecnologias": [
                    "Flask",
                    "spaCy (NLP)",
...
if __name__ == "__main__":
    ...
    log_info("=" * 70)
    log_info(f"Servidor rodando em: http://{host}:{port}")
    log_info("Versão: 1.0-final-validation (COM VALIDAÇÃO DE TEXTO)")
    ...
    log_info(
        "   6.  Semantic Analyzer "
        "(IA + Detecção de Contradição)"
    )
    log_info(
        "   7.  Scorer "
        "(Veracidade + Penalidade por Fake News)"
    )
